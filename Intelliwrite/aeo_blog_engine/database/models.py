...
def _make_entry(content, timestamp=None, is_prompt=None):
    if content is None:
        return None
    if not timestamp:
...
class Blog(Base):
    ...
    @staticmethod
    def make_entry(content, timestamp=None, is_prompt=None):
        return _make_entry(content, timestamp, is_prompt)
